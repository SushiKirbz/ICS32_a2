...
own code. It is raised when attempting to load or save Profile
...
in your own code. It is raised when attempting to deserialize a
...
class Post(dict):
    ...
    The Post class is responsible for working with individual user
    posts. It currently supports two features: A timestamp property
    that is set upon instantiation and when the entry object is set
    and an entry property that stores the post message.
...
class Profile:
    ...
    ensure that a username and password are set, but contains no conventions
    ...
    posts list. Posts are stored in a list object in the order they
    are added. So if multiple Posts objects are created, but added to
    the Profile in a different order, it is possible for the
    list to not be sorted by the Post.timestamp property. So take
    caution as to how you implement your add_post code.
    ...
    operation on the posts returned from the get_posts function to
